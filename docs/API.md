...
(sqrt 2.0)   ;=> 1.41421...
```
Args: `fn-name lib-name c-symbol arg-types ret-type`
...
```
`:structs` is keyed by ABI string, allowing a single file to cover multiple platforms.

**`beer-probe` CLI** — generate binding files from a shell:
```bash
